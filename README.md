# BLE Heart Rate Server (HRS) Sample application

## Overview
The HRS snippet application shows how to initialize and use the WICED BT Heart Rate Server library. This snippet implements the GAP peripheral role. On application start, it calls HRS library APIs to register callbacks for receiving HRC requests. HRS allows a client to register/un-register for notifications and to reset Energy expended values in the server.

Once the application receives the BTM\_ENABLED event, it loads any bonded keys and enters Limited GAP ADV mode, (High duty GAP ADV mode) for 30 seconds, after which it switches to Low duty GAP ADV mode and remains in this mode for 30 seconds. If the client (HRC) does not attempt connection within the timeout, the application exits from ADV mode. The application includes the Heart Rate Service UUID in its ADV data to allow connections from the HRC device. Push the button once to enter Limited GAP ADV mode whenever the HRC wants to connect. After the connection is established with the HRC, the application calls Library APIs to update GATT connection status, and to process HRC GATT read and write requests for Heart Rate notifications. The application is notified by the library whenever HRC configures heart rate notifications. The application stores the bonded HRC address and notification configuration in NVRAM. This information is used in re-connection, after successful encryption, to start heart rate notifications automatically. The application sends heart rate notifications to HRC every 1 minute, until HRC stops.

## Features demonstrated
- Initialize and use the WICED BT HRS library
- GATT DB with Heart Rate notification service and characteristics, Device  Information Service and characteristics
...
4. Application automatically enters Limited GAP ADV mode.
5. After connection with HRC, based on HRC requests, start and stop Heart Rate notifications, and reset the energy expended value accumulated during Heart Rate Notification since the last HRC Energy expended reset request.
6. If HRC is not connected and HRS exited GAP ADV mode, push the application button to enter Limited GAP ADV mode.
